...
static __inline unsigned fls_long(unsigned long l)
{
	if (sizeof(l) == 4)
		return fls(l);
    else
 ...
 */
static __inline unsigned long __ffs64(u64 word)
{
#if BITS_PER_LONG == 32
	if (((u32)word) == 0UL)
 ...
 */
static __inline void generic_set_bit(int nr, volatile unsigned long *addr)
{
...
static __inline void generic_clear_bit(int nr, volatile unsigned long *addr)
{
	unsigned long mask = BIT_MASK(nr);
	unsigned long *p = ((unsigned long *)addr) + BIT_WORD(nr);
