		    ...
		    const nand_erase_options_t *opts)
{
	...
	if ((opts->offset & (mtd->erasesize - 1)) != 0) {
		sysprintf("Attempt to erase non block-aligned data\n");
		return -1;
	}
	...
	for (erased_length = 0;
		...
		if (!opts->scrub) {
			...
			if (ret > 0) {
				if (!opts->quiet)
					sysprintf("\rSkipping bad block at  "
					       "0x%08llx                 "
					       "                         \n",
					       erase.addr);
		...
		if (!opts->quiet) {
			...
			if (percent != percent_complete) {
				...
				sysprintf("\rErasing at 0x%llx -- %3d%% complete.",
				       erase.addr, percent);

				if (opts->jffs2 && result == 0)
					sysprintf(" Cleanmarker written at 0x%llx.",
					       erase.addr);
			}
		}
	}
...
int nand_lock(struct mtd_info *mtd, int tight)
{
	...
	if (chip->read_byte(mtd) & NAND_LOCK_STATUS_TIGHT) {
		sysprintf("nand_lock: Device is locked tight!\n");
		ret = -1;
		goto out;
	}
...
int nand_get_lock_status(struct mtd_info *mtd, loff_t offset)
{
	...
	if ((offset & (mtd->writesize - 1)) != 0) {
		sysprintf("nand_get_lock_status: "
	...
	int allexcept)
{
	...
	if (!(chip->read_byte(mtd) & NAND_STATUS_WP)) {
		sysprintf("nand_unlock: Device is write protected!\n");
		ret = -1;
		goto out;
	}
	...
	chip->cmdfunc(mtd, NAND_CMD_LOCK_STATUS, -1, page & chip->pagemask);
	if (chip->read_byte(mtd) & NAND_LOCK_STATUS_TIGHT) {
		sysprintf("nand_unlock: Device is locked tight!\n");
		ret = -1;
		goto out;
	}

	if ((start & (mtd->erasesize - 1)) != 0) {
		sysprintf("nand_unlock: Start address must be beginning of "
			"nand block!\n");
		ret = -1;
		goto out;
	}
	...
	if (length == 0 || (length & (mtd->erasesize - 1)) != 0) {
		sysprintf("nand_unlock: Length must be a multiple of nand block "
			"size %08x!\n", mtd->erasesize);
		ret = -1;
		goto out;
	}
			...
			size_t *actual, loff_t lim, u_char *buffer, int flags)
{
	 ...
	 */
	if ((offset & (mtd->writesize - 1)) != 0) {
		sysprintf("Attempt to write non page-aligned data\n");
		*length = 0;
		return -EINVAL;
	}
	...
	if (need_skip < 0) {
		sysprintf("Attempt to write outside the flash area\n");
		*length = 0;
		return -EINVAL;
	}
	...
	if (!need_skip && !(flags & WITH_DROP_FFS)) {
		...
		*length = 0;
		sysprintf("NAND write to offset %llx failed %d\n",
			offset, rval);
		return rval;
	}
	...
	while (left_to_write > 0) {
		...
		if (nand_block_isbad(mtd, offset & ~(mtd->erasesize - 1))) {
			sysprintf("Skip bad block 0x%08llx\n",
		       ...
		       size_t *actual, loff_t lim, u_char *buffer)
{
	...
	if ((offset & (mtd->writesize - 1)) != 0) {
		sysprintf("Attempt to read non page-aligned data\n");
		*length = 0;
		if (actual)
			*actual = 0;
		return -EINVAL;
	...
	if (need_skip < 0) {
		sysprintf("Attempt to read outside the flash area\n");
		*length = 0;
		return -EINVAL;
	}
	...
	if (!need_skip) {
		...
		*length = 0;
		sysprintf("NAND read from offset %llx failed %d\n",
			offset, rval);
		return rval;
	}
	...
	while (left_to_read > 0) {
		...
		if (nand_block_isbad(mtd, offset & ~(mtd->erasesize - 1))) {
			sysprintf("Skipping bad block 0x%08llx\n",
				offset & ~(mtd->erasesize - 1));
			offset += mtd->erasesize - block_offset;
			continue;
		}
...
int nand_torture(struct mtd_info *mtd, loff_t offset)
{
	...
	for (i = 0; i < patt_count; i++) {
		...
		if (err) {
			sysprintf("%s: erase() failed for block at 0x%llx: %d\n",
		...
		if ((err && err != -EUCLEAN) || retlen != mtd->erasesize) {
			sysprintf("%s: read() failed for block at 0x%llx: %d\n",
				mtd->name, instr.addr, err);
			goto out;
		}
		...
		if (!err) {
			sysprintf("Erased block at 0x%llx, but a non-0xff byte was found\n",
				offset);
			ret = -EIO;
			goto out;
		}
		...
		if (err || retlen != mtd->erasesize) {
			sysprintf("%s: write() failed for block at 0x%llx: %d\n",
				mtd->name, instr.addr, err);
			goto out;
		}
		...
		if (!err) {
			sysprintf("Pattern 0x%.2x checking failed for block at "
					"0x%llx\n", patterns[i], offset);
			ret = -EIO;
			goto out;
		}
