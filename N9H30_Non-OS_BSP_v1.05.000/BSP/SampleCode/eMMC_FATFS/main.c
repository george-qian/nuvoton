...
void FMI_IRQHandler(void)
{
    ...
    if (inpw(REG_FMI_INTSTS) & FMI_INTSTS_DTAIF_Msk) {
        /* ResetAllEngine() */
        outpw(REG_FMI_CTL, inpw(REG_FMI_CTL) | FMI_CTL_CTLRST_Msk);
        outpw(REG_FMI_INTSTS, FMI_INTSTS_DTAIF_Msk);
    }

	//----- eMMC interrupt status
	isr = inpw(REG_FMI_EMMCINTSTS);
	if (isr & FMI_EMMCINTSTS_BLKDIF_Msk)		// block down
	{
		_fmi_eMMCDataReady = TRUE;
		outpw(REG_FMI_EMMCINTSTS, FMI_EMMCINTSTS_BLKDIF_Msk);
	}

    // CRC error interrupt
    if (isr & FMI_EMMCINTSTS_CRCIF_Msk) {
        if (!(isr & FMI_EMMCINTSTS_CRC16_Msk)) {
            // handle CRC error
        } else if (!(isr & FMI_EMMCINTSTS_CRC7_Msk)) {
            extern unsigned int _fmi_uR3_CMD;
            if (! _fmi_uR3_CMD) {
                // handle CRC error
            }
        }
        outpw(REG_FMI_EMMCINTSTS, FMI_EMMCINTSTS_CRCIF_Msk);      // clear interrupt flag
    }
}
...
int32_t main(void)
{
    ...
    TCHAR		eMMC_path[] = { '2', ':', 0 };    /* eMMC drive started from 0 */
    FRESULT 	res;
    ...
    sysInstallISR(HIGH_LEVEL_SENSITIVE|IRQ_LEVEL_1, FMI_IRQn, (PVOID)FMI_IRQHandler);
    /* enable CPSR I bit */
    sysSetLocalInterrupt(ENABLE_IRQ);
	sysEnableInterrupt(FMI_IRQn);

	/*--- init timer ---*/
	sysSetTimerReferenceClock (TIMER0, 12000000);
	sysStartTimer(TIMER0, 100, PERIODIC_MODE);
	
    sysprintf("\n\nN9H30 FMI eMMC FATFS TEST!\n");
    FMI_SetReferenceClock(300000);
    eMMC_Open_Disk();
	f_chdrive(eMMC_path);          /* set default path */
